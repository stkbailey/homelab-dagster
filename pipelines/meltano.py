...
from dagster import op, job, Failure, ScheduleDefinition
...
@op
def meltano_elt_cmd() -> str:
    """Generates a meltano elt command for the earthquakes pipeline."""
    return "meltano elt tap-gitlab target-jsonl --job_id=gitlab"


@job
def meltano_pipeline():
    run_shell_command(meltano_elt_cmd())



# basic_schedule = ScheduleDefinition(job=meltano_pipeline, cron_schedule="0 0 * * *")
